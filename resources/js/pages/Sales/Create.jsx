...
export default function Create({ shifts, products, employees, authEmployee }) {
    ...
    const filteredProducts = useMemo(() => {
        ...
        if (!currentCabang) return products;
    ...
    // Sinkronkan data.items ketika shift_id berubah
    useEffect(() => {
        if (data.shift_id) {
    ...
    // Hitung modal produk murni dari harga beli produk
    const totalModalProduk = useMemo(() => {
        return data.items.reduce((total, item) => {
            const product = products.find(p => p.id === item.product_id);
            if (product) {
                const q = parseFloat(item.qty) || 0;
    ...
    useEffect(() => {
        ...
        setData('modal_awal', totalModalProduk);
    ...
    }, [totalOmsetProduk, totalModalProduk, autoGaji, data.is_karyawan_hadir, selisihDana]);
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                        ...
                        <div className="mt-6 border-t border-slate-200 dark:border-slate-700 pt-4">
                            {!isKaryawan && (
                                <label className="flex items-center gap-2 cursor-pointer p-3 bg-slate-50 dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-800 hover:border-blue-300 transition-colors">
                                    <input
                                        type="checkbox"
                                        id="admin_jualan_checkbox_sales"
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                                <div>
                                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                                        Nama Karyawan {data.is_karyawan_hadir === true ? <span className="text-red-500">*</span> : <span className="text-slate-400 font-normal ml-1">(Opsional)</span>}
                                    </label>
                                    {isKaryawan ? (
                                        <input
                                            type="text"
                                        ...
                                        <select
                                            ...
                                            className={`${inputClasses} ${data.is_karyawan_hadir === false ? 'bg-white dark:bg-slate-800 opacity-80' : ''}`}
                                    ...
                                    <div className="flex items-end pb-1">
                                        <div className="text-sm text-slate-500 dark:text-slate-400">
                                            Status: <span className={data.is_karyawan_hadir === true ? "text-blue-600 font-bold" : "text-emerald-600 font-bold"}>{data.is_karyawan_hadir === true ? "Karyawan Hadir" : "Admin Jualan Sendiri"}</span>
                                        </div>
                                    </div>
                                    {isKaryawan && (
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                            ...
                            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-emerald-600 dark:text-emerald-400 mt-1">= Σ(Harga Beli × Qty)</p>
                                    </div>

                                    <div>
                                        <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">💰 Modal Harian (Manual)</label>
                                        ...
                                        <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1">{formatRp(data.modal_harian)} (Operasional)</p>
                                    </div>
                        ...
                        <div className="lg:col-span-4 bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col h-[500px] lg:h-[800px]">
                            <div className="p-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50 flex items-center justify-between">
                                ...
                                {selectedShift && (
                                    <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300">
                            ...
                            <div className="p-4 flex-1 overflow-y-auto space-y-4">
                                {!data.shift_id ? (
                                    <div className="py-16 text-center text-gray-500 dark:text-gray-400">
                                        <p className="text-sm font-semibold">Pilih Cabang / Shift Terlebih Dahulu</p>
                                        <p className="text-xs mt-1 text-gray-400">Menu makanan akan tampil sesuai merk cabang.</p>
                                    </div>
                                ) : filteredProducts.length === 0 ? (
                                    <div className="py-16 text-center text-gray-500 dark:text-gray-400">
                                        <p className="text-sm font-semibold">Tidak ada produk untuk merk "{currentCabang}"</p>
                                        <p className="text-xs mt-1 text-gray-400">Silakan cek kategori dan produk di Master Data.</p>
                                    </div>
                                ) : (
                                    filteredProducts.map(product => {
                                        ...
                                        return (
                                            <div key={product.id} className={`flex items-center justify-between p-3.5 rounded-2xl border transition-all duration-300 group ${isOverStock ? 'border-red-300 dark:border-red-800 bg-red-50/50 dark:bg-red-900/20' : 'border-slate-200 dark:border-slate-700 hover:border-blue-300 dark:hover:border-blue-700 hover:bg-blue-50/50 dark:hover:bg-blue-900/20 hover:shadow-md hover:shadow-blue-500/5'
                                                }`}>
                                                ...
                                                <div className="w-24 ml-2">
                                                    <input
                                                        ...
                                                        className={`w-full text-center rounded-xl border py-2 font-bold focus:ring-2 focus:bg-white dark:focus:bg-slate-800 transition-all shadow-sm ${isOverStock
